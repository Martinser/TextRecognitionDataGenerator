...
def equal_distribution_combinations(iterable, n, csize,decide,ln):
    '''
       均匀生成一定范围内的随机数
    '''
    ...
    for i in range(n):
        ...
        if decide==1:
            if ln==1:
                csize=random.randint(1,1)#生成单字图像
            else:
                csize=random.randint(1,30)#生成多字图像
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=(0, 0, 0, 0),#margin_top, margin_left, margin_bottom, margin_right  #default=(5, 5, 5, 5)
    )
    #，左
    parser.add_argument(
        "-fi",
...
def main():
    ...
    string_wg=[]
    items = range(14404)#14404为B榜码表去除Unicode 扩展E~F之后字符的数量
    item_counts = {k: 0 for k in items}
    temp = random.randint(2, 30)
    for comb in equal_distribution_combinations(items, args.count, temp,decide=1,ln=args.length):#decide为1时候进行随机
        temp_string=''
        for e in comb:
            item_counts[e] += 1
            temp_string=temp_string+lang_dict[e]
        string_wg.append(temp_string)

    strings.clear()
    strings=string_wg

    for vs in range(len(strings)):
        strings[vs]=strings[vs].replace(' ','')

    width_wg = []#生成的均匀随机宽度
    string_width=range(0,3)#字符宽度有3种均匀随机变换
    item_counts2 = {k: 0 for k in string_width}
    for comb in equal_distribution_combinations(string_width, args.count, 1,decide=0,ln=args.length):#decide为0时候不进行随机
        for e in comb:
            item_counts2[e] += 1
            width_wg.append(e)
    ...
    fonts_wg = []#生成的均匀随机字体
    string_font = range(0, 6)#字体有6种均匀随机变换
    item_counts3 = {k: 0 for k in string_font}
    for comb in equal_distribution_combinations(string_font, args.count, 1, decide=0,ln=args.length):  # decide为0时候不进行随机
        for e in comb:
            item_counts3[e] += 1
            fonts_wg.append(e)
